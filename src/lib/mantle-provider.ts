// Deterministic demo Mantle provider. Ported from the Hono backend's
// MockMantleProvider — curated, intentionally partial data used to showcase
// source-aware holder / concentration / liquidity intelligence.
...
export const SENTINEL_DEMO_METADATA: SentinelMetadata = {
...
type SummarySeed = Pick<
  MantleAssetSummary,
  "symbol" | "name" | "price_usd" | "price_7d_high" | "price_7d_low" | "liquidity_usd" | "holder_count" | "top_holder_percent"
>;

const assetSummaries = new Map<string, SummarySeed>([
  ["0x1111111111111111111111111111111111111111", { symbol: "mBURROW", name: "Mock Burrow Signal Token", price_usd: "0.56", price_7d_high: "0.62", price_7d_low: "0.48", liquidity_usd: "4285000.00", holder_count: 1284, top_holder_percent: "18.42" }],
  ["0x2222222222222222222222222222222222222222", { symbol: "mDEMO", name: "Mock Demo Liquidity Token", price_usd: "0.14", price_7d_high: "0.16", price_7d_low: "0.12", liquidity_usd: "1918000.00", holder_count: 892, top_holder_percent: "14.20" }],
  ["0x3333333333333333333333333333333333333333", { symbol: "WBTC", name: "Wrapped Bitcoin", price_usd: "64250.00", price_7d_high: "67800.00", price_7d_low: "61200.00", liquidity_usd: "9825000.00", holder_count: 3412, top_holder_percent: "15.80" }],
  ["0x4444444444444444444444444444444444444444", { symbol: "mETH", name: "Mantle Staked Ether", price_usd: "3180.00", price_7d_high: "3340.00", price_7d_low: "2980.00", liquidity_usd: "7440000.00", holder_count: 5821, top_holder_percent: "22.10" }],
  ["0x5555555555555555555555555555555555555555", { symbol: "USDY", name: "Ondo US Dollar Yield", price_usd: "1.04", price_7d_high: "1.05", price_7d_low: "1.02", liquidity_usd: "5360000.00", holder_count: 2147, top_holder_percent: "31.20" }],
  ["0x6666666666666666666666666666666666666666", { symbol: "MNT", name: "Mantle", price_usd: "0.82", price_7d_high: "0.91", price_7d_low: "0.74", liquidity_usd: "12840000.00", holder_count: 14829, top_holder_percent: "24.80" }],
]);

type ConcentrationSeed = MantleConcentrationResponse["metrics"];

const concentrationMap = new Map<string, ConcentrationSeed>([
...
export function getAssetSummary(address: string): MantleAssetSummary {
  const normalizedAddress = normalizeAddress(address);
  const asset =
    assetSummaries.get(normalizedAddress) ?? {
      symbol: "MANTLE",
      name: "Mantle asset",
      price_usd: "0.00",
      price_7d_high: null,
  ...
  return {
    ...
    metadata: SENTINEL_DEMO_METADATA,
  };
}

export function getAssetHolders(address: string, options: { limit: number }): MantleHoldersResponse {
  const normalizedAddress = normalizeAddress(address);
  const assetHolders =
    holdersMap.get(normalizedAddress) ?? holdersMap.get("0x1111111111111111111111111111111111111111")!;

  return {
    chain: "mantle",
    asset_address: normalizedAddress,
    holders: assetHolders.slice(0, options.limit),
    page: { limit: options.limit, next_cursor: null },
    metadata: SENTINEL_DEMO_METADATA,
  };
}

export function getAssetConcentration(address: string): MantleConcentrationResponse {
  const normalizedAddress = normalizeAddress(address);
  const metrics =
    concentrationMap.get(normalizedAddress) ?? {
      top_1_percent: "18.42",
      top_5_percent: "41.73",
  ...
  return { chain: "mantle", asset_address: normalizedAddress, metrics, metadata: SENTINEL_DEMO_METADATA };
}

export function getLiquidityDelta(): LiquidityDeltaResponse {
  return {
    chain: "mantle",
    signals: [
      { asset_address: "0x1111111111111111111111111111111111111111", symbol: "mBURROW", window: "24h", liquidity_delta_usd: "284000.00", liquidity_delta_percent: "7.09", signal: "inflow", confidence: "medium" },
      { asset_address: "0x2222222222222222222222222222222222222222", symbol: "mDEMO", window: "24h", liquidity_delta_usd: "-42000.00", liquidity_delta_percent: "-2.14", signal: "outflow", confidence: "low" },
      { asset_address: "0x4444444444444444444444444444444444444444", symbol: "mETH", window: "24h", liquidity_delta_usd: "512000.00", liquidity_delta_percent: "6.88", signal: "inflow", confidence: "high" },
      { asset_address: "0x5555555555555555555555555555555555555555", symbol: "USDY", window: "24h", liquidity_delta_usd: "89000.00", liquidity_delta_percent: "1.66", signal: "inflow", confidence: "medium" },
      { asset_address: "0x6666666666666666666666666666666666666666", symbol: "MNT", window: "24h", liquidity_delta_usd: "-340000.00", liquidity_delta_percent: "-2.65", signal: "outflow", confidence: "high" },
    ],
    metadata: SENTINEL_DEMO_METADATA,
  };
}
