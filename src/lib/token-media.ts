// Resolves curated token logos shipped under /public/media. Pure lookup logic,
...
export function resolveTokenMediaFile(input: TokenMediaLookupInput): string | null {
  ...
  if (address && mediaByAddress[address]) return mediaByAddress[address];

  const symbol = normalizeLookupValue(input.symbol);
  if (symbol && mediaBySymbol[symbol]) return mediaBySymbol[symbol];

  const slug = normalizeLookupValue(input.slug);
  if (slug && mediaBySymbol[slug]) return mediaBySymbol[slug];

  const name = normalizeLookupValue(input.name);
  if (name && mediaByName[name]) return mediaByName[name];
